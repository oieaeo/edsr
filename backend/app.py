...
# ===================================================================
# FUNGSI UTILITAS (Diadaptasi dari skrip asli)
# ===================================================================
...
def merge_audio(temp_video, original_video, output_video):
    ...
    try:
        command = [
            ...
            "-i", original_video,
            "-c:v", "copy",
            "-c:a", "aac", # Gunakan codec audio yang umum
            "-map", "0:v:0",
            "-map", "1:a:0?", # Tanda tanya agar tidak error jika tidak ada audio
            "-shortest",
            output_video
        ]
    ...
    if os.path.exists(HISTORY_FILE):
        with open(HISTORY_FILE, 'r') as f:
            try:
                ...
                history = []
    history.insert(0, data) # Tambah item baru di awal
...
def process_video_task(session_id, original_video_path, original_filename):
    ...
    try:
        ...
        # 3. Siapkan video output (tanpa audio terlebih dahulu)
        ...
        metrics = {
            'psnr_avg': f"{np.mean(psnr_scores):.2f}",
            'ssim_avg': f"{np.mean(ssim_scores):.4f}",
            'psnr_per_frame': psnr_scores,
            'ssim_per_frame': ssim_scores
        }
        
        final_result = {
            'enhanced_video_url': f'/static/enhanced/{final_video_filename}',
        ...
        history_data = {
            ...
            'original_video_url': f'/static/uploads/{os.path.basename(original_video_path)}',
            'enhanced_video_url': f'/static/enhanced/{final_video_filename}',
            'processing_time': str(processing_time),
            'metrics': {
...
def process_endpoint():
    ...
    SESSIONS[session_id] = {
        'progress': 0, 'status': 'Waiting to start...', 'error': None,
        'final_result': None, 'current_psnr': 0, 'current_ssim': 0
    }
    
    thread = threading.Thread(
        target=process_video_task, 
        args=(session_id, video_path, file.filename)
    )
    thread.start()
...
# --- PERBAIKAN ---
# Endpoint diubah untuk menerima POST dan mencari item berdasarkan identifier.
# Ini memastikan item lama (tanpa ID) dan item baru (dengan ID) dapat dihapus.
@app.route('/history/delete', methods=['POST'])
def delete_history_item():
    """Menghapus item riwayat dan file video terkait berdasarkan identifier."""
    data = request.get_json()
    identifier = data.get('identifier')

    if not identifier:
        return jsonify({'error': 'Identifier diperlukan'}), 400

    if not os.path.exists(HISTORY_FILE):
        return jsonify({'message': 'File riwayat tidak ditemukan.'}), 200
    ...
    # Cari item yang akan dihapus dan indeksnya
    item_to_delete = None
    index_to_delete = -1
    for i, item in enumerate(history):
        # Cek berdasarkan ID dulu (untuk item baru), lalu berdasarkan URL (untuk item lama)
    ...
    # Hapus file fisik (video) yang terkait
    try:
        if 'original_video_url' in item_to_delete and item_to_delete['original_video_url']:
            original_file_path = os.path.join(BASE_DIR, item_to_delete['original_video_url'].lstrip('/\\'))
            if os.path.exists(original_file_path):
                os.remove(original_file_path)
                logger.info(f"🗑️ File asli berhasil dihapus: {original_file_path}")

        if 'enhanced_video_url' in item_to_delete and item_to_delete['enhanced_video_url']:
            enhanced_file_path = os.path.join(BASE_DIR, item_to_delete['enhanced_video_url'].lstrip('/\\'))
            if os.path.exists(enhanced_file_path):
                os.remove(enhanced_file_path)
                logger.info(f"🗑️ File hasil proses berhasil dihapus: {enhanced_file_path}")
    except Exception as e:
        logger.error(f"❌ Terjadi kesalahan saat menghapus file untuk identifier {identifier}: {e}")
        # Proses tetap lanjut untuk menghapus entri dari riwayat

    # Hapus item dari daftar riwayat
    del history[index_to_delete]

    # Tulis kembali file riwayat yang sudah diperbarui
    with open(HISTORY_FILE, 'w') as f:
        json.dump(history, f, indent=4)
...
if __name__ == '__main__':
    ...
    logger.info("🚀 Server AI VideoStream siap di http://127.0.0.1:5001")
